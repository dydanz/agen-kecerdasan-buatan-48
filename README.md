...
DM the bot or use `/ask <message>` in any server channel (ephemeral).

To enable `@mention` response in server channels, set `mention_response = true` under `[adapters.discord]` — the bot will reply threaded to the original mention.
