...
name: github-pr
description: >
  Create and manage GitHub PRs for the AKB48 project with full metadata:
...
gh project item-add ${AKB48_PROJECT_NUM} \
  --owner dydanz \
...
gh project item-edit --project-id ${AKB48_PROJECT_ID} --id <ITEM_ID> \
  --field-id ${AKB48_FIELD_PRIORITY} --single-select-option-id <OPTION_ID>
```
...
# Values: "XS" | "S" | "M" | "L" | "XL"
gh project item-edit --project-id ${AKB48_PROJECT_ID} --id <ITEM_ID> \
  --field-id ${AKB48_FIELD_SIZE} --single-select-option-id <OPTION_ID>
```
...
# Values: 1 | 2 | 3 | 5 | 8 | 13
gh project item-edit --project-id ${AKB48_PROJECT_ID} --id <ITEM_ID> \
  --field-id ${AKB48_FIELD_ESTIMATE} --number <SP>
```

### Start Date / Target Date
```bash
gh project item-edit --project-id ${AKB48_PROJECT_ID} --id <ITEM_ID> \
  --field-id ${AKB48_FIELD_START_DATE} --date "YYYY-MM-DD"

gh project item-edit --project-id ${AKB48_PROJECT_ID} --id <ITEM_ID> \
  --field-id ${AKB48_FIELD_TARGET_DATE} --date "YYYY-MM-DD"
```
...
```
AKB48_PROJECT_NUM=1
AKB48_PROJECT_ID=PVT_xxxxxxxxxxxx
AKB48_FIELD_PRIORITY=PVTF_xxxxxxxxxxxx
AKB48_FIELD_SIZE=PVTF_xxxxxxxxxxxx
AKB48_FIELD_ESTIMATE=PVTF_xxxxxxxxxxxx
AKB48_FIELD_START_DATE=PVTF_xxxxxxxxxxxx
AKB48_FIELD_TARGET_DATE=PVTF_xxxxxxxxxxxx
```
...
- [ ] Body has: Summary, Risk, Rollback, Test plan, `Closes #N`, `KLW: #N`
...
- `adr-required` — write `akb48-adr/ADR-*.md` within 48h of merge
- Implementation deviated from plan — update `akb48-dev-plan/phase-*.md`
